...
internal static class VamanaGraphIO
{
        ...
        IReadOnlyCollection<int> tombstones)
    {
        ...
        {
            throw new DotVectorException($"非法 MaxDegree：{options.MaxDegree}。");
        }
        ...
        for (int row = 0; row < neighbors.Count; row++)
        {
            ...
            for (int i = 0; i < nbr.Length; i++)
            {
                ...
                if (n < 0)
                {
                    throw new DotVectorException($"row {row} 出现负邻居 id {n}。");
                }
        ...
        out HashSet<int> tombstones)
    {
        ...
        if (!File.Exists(path))
        {
            throw new DotVectorException($"vamana.bin 不存在：{path}");
        }

        FileInfo fi = new(path);
        if (fi.Length < HeaderSize)
        {
            throw new DotVectorException($"vamana.bin 损坏：长度 {fi.Length} 小于头部 {HeaderSize}。");
        }
        ...
        {
            throw new DotVectorException("vamana.bin 损坏：Magic 不匹配（期望 'DVAN'）。");
        }
        ...
        {
            throw new DotVectorException($"vamana.bin 损坏：非法 MaxDegree {header.MaxDegree}。");
        }
