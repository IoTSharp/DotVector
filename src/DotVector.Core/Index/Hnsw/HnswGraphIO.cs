...
internal static class HnswGraphIO
{
        ...
        out HashSet<int> tombstones)
    {
        ...
        if (nodeCount < 0) throw new DotVectorException($"hnsw.bin: 非法 NodeCount={nodeCount}。");
        ...
        for (int row = 0; row < nodeCount; row++)
        {
            if (fs.Read(nodeHdrBuf) != nodeHdrBuf.Length)
                throw new DotVectorException($"hnsw.bin: row {row} node 头不完整。");
            int level = BinaryPrimitives.ReadInt32LittleEndian(nodeHdrBuf.Slice(0, 4));
            byte tombstone = nodeHdrBuf[4];
            if (level < 0)
                throw new DotVectorException($"hnsw.bin: row {row} 非法 level={level}。");
            ...
            for (int layer = 0; layer <= level; layer++)
            {
                if (fs.Read(uintBuf) != uintBuf.Length)
                    throw new DotVectorException($"hnsw.bin: row {row} layer {layer} 计数缺失。");
                int count = (int)BinaryPrimitives.ReadUInt32LittleEndian(uintBuf);
                if (count < 0 || count > nodeCount)
                    throw new DotVectorException($"hnsw.bin: row {row} layer {layer} 非法 neighbor 数 {count}。");
                ...
                if (count > 0)
                {
                    ...
                    if (fs.Read(bytes, 0, bytes.Length) != bytes.Length)
                        throw new DotVectorException($"hnsw.bin: row {row} layer {layer} 邻居体不完整。");
                    Span<byte> bs = bytes;
                    for (int i = 0; i < count; i++)
                    {
                        int n = BinaryPrimitives.ReadInt32LittleEndian(bs.Slice(i * sizeof(int), sizeof(int)));
                        if (n < 0 || n >= nodeCount)
                            throw new DotVectorException($"hnsw.bin: row {row} layer {layer} neighbor #{i}={n} 越界。");
                        nbr[i] = n;
                    }
                }
                rowLayers[layer] = nbr;
