...
internal static class IvfFlatGraphIO
{
        ...
        using (var fs = new FileStream(tmp, FileMode.Create, FileAccess.Write, FileShare.None))
        {
            ...
            if (snapshot.IsTrained)
            {
                if (snapshot.Centroids is null || snapshot.InvertedLists is null)
                    throw new DotVectorException("ivfflat.bin: snapshot 已训练但缺少 centroids 或 inverted lists。");
                if (snapshot.Centroids.Length != checked(snapshot.Options.NList * snapshot.Dimensions))
                    throw new DotVectorException("ivfflat.bin: centroids 长度与 NList × Dimensions 不一致。");
    ...
    {
        ArgumentNullException.ThrowIfNull(path);
        if (!File.Exists(path)) throw new DotVectorException($"ivfflat.bin 不存在：{path}");

        using var fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read);
        Span<byte> hdr = stackalloc byte[HeaderSize];
        if (fs.Read(hdr) != HeaderSize)
            throw new DotVectorException("ivfflat.bin 损坏：header 不完整。");

        if (!hdr.Slice(0, 8).SequenceEqual(MagicAscii))
            throw new DotVectorException("ivfflat.bin 损坏：Magic 不匹配（期望 'DIVF-F'）。");
        uint version = BinaryPrimitives.ReadUInt32LittleEndian(hdr.Slice(8, 4));
        if (version != CurrentVersion)
            throw new DotVectorException($"ivfflat.bin 不支持的格式版本 {version}（期望 {CurrentVersion}）。");
        ...
        if (dimensions <= 0 || nList <= 0 || rowCount < 0)
            throw new DotVectorException("ivfflat.bin 损坏：非法 header 字段。");
        ...
        if (isTrained)
        {
            ...
            for (int i = 0; i < nList; i++)
            {
                if (fs.Read(uintBuf) != uintBuf.Length)
                    throw new DotVectorException($"ivfflat.bin: list {i} 长度缺失。");
                int len = (int)BinaryPrimitives.ReadUInt32LittleEndian(uintBuf);
                if (len < 0 || len > rowCount)
                    throw new DotVectorException($"ivfflat.bin: list {i} 非法长度 {len}。");
    ...
    private static int[] ReadIntArray(FileStream fs, int count)
    {
        ...
        if (fs.Read(buf, 0, buf.Length) != buf.Length)
            throw new DotVectorException("ivfflat.bin: int 数组读取不完整。");
    ...
    private static float[] ReadFloatArray(FileStream fs, int count)
    {
        ...
        if (fs.Read(buf, 0, buf.Length) != buf.Length)
            throw new DotVectorException("ivfflat.bin: float 数组读取不完整。");
