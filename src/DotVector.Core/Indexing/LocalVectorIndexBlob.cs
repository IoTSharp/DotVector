...
public static class LocalVectorIndexBlob
{
    ...
    public static uint Write(Stream stream, IVectorIndexReader reader)
    {
        ...
        if (reader is not LocalVectorIndexReader { Index: HnswIndex<int> hnswIndex } localReader)
            throw new NotSupportedException("Only DotVector local HNSW index readers can be serialized as index blobs.");
    ...
    /// <summary>
    /// 从 blob 读取 DotVector 本地索引 reader。
    /// </summary>
    public static IVectorIndexReader Read(Stream stream, int length, uint expectedCrc32)
    {
        ...
        if (actualCrc32 != expectedCrc32)
            throw new InvalidDataException($"DotVector index blob CRC32 mismatch: expected 0x{expectedCrc32:X8}, got 0x{actualCrc32:X8}.");

        ReadOnlySpan<byte> header = blob.AsSpan(0, HeaderSize);
        if (!header[..8].SequenceEqual(Magic))
            throw new InvalidDataException("DotVector index blob magic mismatch.");

        int version = BinaryPrimitives.ReadInt32LittleEndian(header[8..12]);
        if (version != FormatVersion)
            throw new InvalidDataException($"Unsupported DotVector index blob version: {version}.");

        int headerSize = BinaryPrimitives.ReadInt32LittleEndian(header[12..16]);
        if (headerSize != HeaderSize)
            throw new InvalidDataException($"Invalid DotVector index blob header size: {headerSize}.");
        ...
        if (algorithm != VectorIndexAlgorithm.Hnsw)
            throw new InvalidDataException($"Unsupported DotVector index blob algorithm: {algorithm}.");
        ...
        HnswOptions options)
    {
        ...
        if (offset + vectorBytes > payload.Length)
            throw new InvalidDataException("DotVector HNSW blob vector payload is truncated.");
        ...
        if (rowCount != count)
            throw new InvalidDataException("DotVector HNSW blob neighbor row count does not match metadata.");
        ...
        if (offset != payload.Length)
            throw new InvalidDataException("DotVector HNSW blob contains trailing bytes.");
    ...
    private static Metric ToMetric(KnnMetric metric)
        => metric switch
        {
            ...
            _ => throw new InvalidDataException($"Unsupported DotVector index blob metric: {metric}."),
        };
    ...
    private static int ReadInt32(ReadOnlySpan<byte> source, ref int offset)
    {
        if (offset + sizeof(int) > source.Length)
            throw new InvalidDataException("DotVector HNSW blob is truncated.");
    ...
    private static void FillBuffer(Stream stream, Span<byte> buffer)
    {
        ...
        while (readTotal < buffer.Length)
        {
            ...
            if (read == 0)
                throw new InvalidDataException("DotVector index blob is truncated.");
            readTotal += read;
        }
    }
}
