    ...
    public DotVectorRecordMapper()
    {
        ...
        foreach (var p in props)
        {
            if (p.GetCustomAttribute<VectorStoreKeyAttribute>() is not null)
            {
                if (key is not null)
                {
                    throw new InvalidOperationException(
                        $"{t.FullName} 上声明了多个 [VectorStoreKey] 属性，DotVector.VectorData 仅支持单主键。");
                }
            ...
            if (p.GetCustomAttribute<VectorStoreVectorAttribute>() is { } vAttr)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"{t.FullName} 上声明了多个 [VectorStoreVector] 属性，DotVector.VectorData 当前仅支持单向量字段。");
                }
                vector = p;
        ...
        else
        {
            throw new NotSupportedException(
                ...
                " DotVector.VectorData 当前仅支持 float[] 与 ReadOnlyMemory<float>。");
        }
        ...
        foreach (var dp in definition.Properties)
        {
            ...
            switch (dp)
            {
                case VectorStoreKeyProperty:
                    ...
                    {
                        throw new InvalidOperationException(
                            $"VectorStoreCollectionDefinition 中存在多个 Key 属性，DotVector.VectorData 仅支持单主键。");
                    }
                ...
                case VectorStoreVectorProperty vp:
                    ...
                    {
                        throw new InvalidOperationException(
                            $"VectorStoreCollectionDefinition 中存在多个 Vector 属性，DotVector.VectorData 仅支持单向量字段。");
                    }
                    vector = clr;
                    dims = vp.Dimensions;
