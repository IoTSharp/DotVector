...
/// <summary>
/// HNSW 是近似（approximate）检索；M3 验收要求 Recall@10 ≥ 0.95。
/// </summary>
public sealed class HnswRecallTests
{
    [Theory]
    ...
    public void HnswRecall_At10_AtLeast95Percent(Metric metric, int seed)
    {
        const int N = 1000;
        const int Dim = 64;
        const int TopK = 10;
        ...
        var hnswOptions = new HnswOptions
        {
            M = 16,
            EfConstruction = 200,
            EfSearch = 100,
        ...
        var c = db.CreateCollection<int>("recall", Dim, metric, IndexKind.Hnsw, hnswOptions);
        for (int i = 0; i < N; i++)
        {
        ...
        Assert.True(recall >= 0.95, $"Recall@{TopK} = {recall:F3}, 期望 ≥ 0.95（metric={metric}, seed={seed}）。");
    }
}
